# the dataset is modified based on promptir (https://github.com/va1shn9v/PromptIR) and airnet (https://github.com/XLearning-SCU/2022-CVPR-AirNet/tree/main) with a similar processing procedure. 
...
class PromptTrainDataset(Dataset):
    '''by default noise_combine is True while False for promptir'''
    ...
    def _init_blur_ids(self):
        temp_ids = []
        image_list = os.listdir(os.path.join(self.args.gopro_dir, 'train/sharp/'))
        temp_ids = image_list
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            ...
            if de_id == 3:
                ...
                # GOPRO debluring dataset
                degrad_img = crop_img(np.array(Image.open(os.path.join(self.args.gopro_dir, 'train/blur', sample["clean_id"])).convert('RGB')), base=16)
                clean_img = crop_img(np.array(Image.open(os.path.join(self.args.gopro_dir, 'train/sharp', sample["clean_id"])).convert('RGB')), base=16)
...
class DeblurTestDataset(Dataset):
    ...
    def __init__(self, args, addnoise = False, sigma = None, is_test = True, is_val = False):
        super().__init__()
        temp_ids = []
        image_list = os.listdir(os.path.join(args.gopro_dir, 'test/sharp/'))
        temp_ids = image_list
    ...
    def __getitem__(self, idx):
        ...
        degrad_img = crop_img(np.array(Image.open(os.path.join(self.args.gopro_dir, 'test/blur', sample["clean_id"])).convert('RGB')), base=16)
        clean_img = crop_img(np.array(Image.open(os.path.join(self.args.gopro_dir, 'test/sharp', sample["clean_id"])).convert('RGB')), base=16)
        clean_name = sample["clean_id"]
